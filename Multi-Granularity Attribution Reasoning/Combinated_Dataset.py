...
def load_and_shuffle_dataset(config):
    ...
    train_data = load_json(config["pheme_train_json_name"])
    test_data = load_json(config["pheme_test_json_name"])
    val_data = load_json(config["pheme_val_json_name"])
    ...
    dataset_train = MGDataset(df_train, config["root_dir"], "unique_id", "unique_id", config["pheme_image_vec_dir"],
                                 config["pheme_text_vec_dir"], config["pheme_event_vec_dir"], config["pheme_evidence_vec_dir"])
    dataset_val = MGDataset(df_val, config["root_dir"], "unique_id", "unique_id", config["pheme_image_vec_dir"],
                               config["pheme_text_vec_dir"], config["pheme_event_vec_dir"], config["pheme_evidence_vec_dir"])
    dataset_test = MGDataset(df_test, config["root_dir"], "unique_id", "unique_id", config["pheme_image_vec_dir"],
                                config["pheme_text_vec_dir"], config["pheme_event_vec_dir"], config["pheme_evidence_vec_dir"])
...
def load_npy_file(config):
    if os.path.exists(config['cache_path']):
        ...
        sentence_embedding = torch.load(config['cache_path'] + "/Seg_Text_Embedding_File/pheme_sentence_embeddings.pt")

        with open(f"{config['cache_path']}/entity_mapping.json", 'r', encoding='utf-8') as f:
