...
"""
这个函数 `prepare_device` 的作用是配置和准备用于深度学习训练的GPU设备。具体功能如下：
1. 接收参数 `n_gpu_use`，表示想要使用的GPU数量
2. 检测系统中实际可用的GPU数量（通过 `torch.cuda.device_count()`）
3. 进行两项检查：
   - 如果想使用GPU但实际没有GPU，发出警告并切换到CPU训练
   - 如果请求的GPU数量超过可用数量，发出警告并调整为实际可用数量
4. 创建适当的设备对象：
   - 如果有GPU可用则使用 `cuda:0`（第一个GPU）
   - 否则使用 `cpu`
5. 创建一个包含所有将要使用的GPU索引的列表（用于PyTorch的 `DataParallel`）
6. 返回设备对象和GPU索引列表

这个函数是深度学习项目中的常用工具，确保代码能在不同硬件环境下优雅地运行，无论是有多个GPU、单个GPU还是只有CPU的情况。
"""
def prepare_device(n_gpu_use):
    ...
    if n_gpu_use > n_gpu:
        ...
        n_gpu_use = n_gpu
    device = torch.device('cuda:5' if n_gpu_use > 0 else 'cpu')
...
def build_item2item_dataset(config):
    ...
    fp_train.close()
    fp_valid.close()
    user_history_dict = {} # 每位用户的“历史记录点击”新闻 ID，包括“历史记录”和“行为”
    news_click_dict = {} # 所有用户点击每条新闻的总点击次数
    doc_doc_dict = {} # 所有用户点击每条新闻的总点击次数, (doc1,doc2)和(doc2,doc1) 是相同的
    """用来维护两个重要的字典，第一个是记录每位用户历史新闻交互记录的字典，另一个是统计每条新闻的总的点击数的字典"""
    for line in lines:
...
def build_neibor_embedding(config, entity_doc_dict, doc_feature_embedding, entity_id_dict): # 返回每个实体的 Doc Embedding sum 和 num-1，用于计算相干奖励
