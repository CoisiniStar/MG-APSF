...
class HeteroEmbeddingLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.heterographlayers = nn.ModuleDict({f"layer{i}": dglnn.HeteroGraphConv({
            ...
            'text_edge': conv.GATConv(in_feats[i], out_feats[i],
                                      ...
                                      attn_drop=0.0,
                                      residual=True, activation=nn.ELU(), bias=True),
            'event_edge': conv.GATConv(in_feats[i], out_feats[i],
                                       num_heads=1,
                                        feat_drop=0.4,
                                        attn_drop=0.0,
                                        residual=True, activation=nn.ELU(), bias=True)
        }, aggregate='mean') for i in range(len(in_feats))}).to(config["gnn_device"])
    ...
    def forward(self, g):
        ...
        x_features = {
            ...
            'event_node': g.nodes['event_node'].data['features']
        }
        ...
        x_features['event_node'] = self.mm_embedding_space(x_features['event_node'])
...
class HeteroGraphClassifier(nn.Module):
    def __init__(self, config,
        ...
        self.config = config
        self.device = config["gnn_device"]
        ## Inidividual GAT layers
        self.heteroembeddinglayers = HeteroEmbeddingLayer(
                                                          config=self.config,
                                                          in_feats=in_feats_embedding,
    ...
    def forward(self, g):
        ...
        with g.local_scope():
            ...
            g.nodes['event_node'].data['features'] = x_features['event_node']
